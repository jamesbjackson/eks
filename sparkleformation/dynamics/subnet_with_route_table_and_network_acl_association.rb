SparkleFormation.dynamic(:subnet_with_route_table_and_network_acl_association) do |_name, _config={}|
  ...
  parameters do
    ...
    set!("#{subnet_name}_network_acl_id".to_sym) do
      description 'The ID of the network ACL to associate with the subnet.'
      registry!(:string_aws_parameter_type)
    end  
  ...
  resources("#{subnet_name}_subnet_network_acl_association".to_sym) do
    type 'AWS::EC2::SubnetNetworkAclAssociation'
    properties do
      network_acl_id ref!("#{subnet_name}_network_acl_id".to_sym)
      subnet_id ref!(subnet_name.to_sym)
    end
    depends_on depends_on!(subnet_name.to_sym)
  end
  ...
  outputs do
    ...
    set!("#{subnet_name}_subnet_network_acl_association_id".to_sym) do
      description 'The ID of the subnet network acl association'
      value ref!("#{subnet_name}_subnet_network_acl_association".to_sym)
    end
  end
end
