SparkleFormation.new(:eks_load_balancer_subnets, compile_time_parameters: {
  ...
  kubernetes_clusters: { type: :string, multiple: true },
  shared_tags: { type: :string, multiple: true }
}).load(:base).overrides do
description "EC2 - EKS Managed Subnets"
  ...
  state!(:resource_name).each_with_index do |name, i|
    ...
    state!(:kubernetes_clusters).each do |tag_name|
      subnet_tags << {Key: "kubernetes.io/cluster/" + tag_name, Value: "shared"}
    end

    state!(:shared_tags).each do |tag|
      name, val = tag.split('=')
